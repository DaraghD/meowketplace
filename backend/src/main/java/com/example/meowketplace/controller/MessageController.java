...
public class MessageController {
    ...
    public ResponseEntity<List<Message>> getMessages(@RequestHeader("Authorization") String authHeader, @PathVariable Long sender, @PathVariable Long receiver) {
    ...
    public ResponseEntity<String> deleteMessage(@RequestHeader("Authorization") String authHeader, @PathVariable Long deleteId) {
        try {
            messageService.deleteMessage(authHeader, deleteId); //auth header to see if message is owned by the auth user
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
        return ResponseEntity.status(HttpStatus.OK).body("Message sent?");
